...
2. **ClassifyByDigitSum(values)**: Given a sequence of non-negative integers, return two sequences: one containing values whose digit sum is even, and one containing values whose digit sum is odd. Every element of the input must appear in exactly one of the two output sequences.
...
- **No fabrication**: The combined length of both outputs equals the length of the input.
