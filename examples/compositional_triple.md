...
2. **FilterByFrequency(freq, threshold)**: Given a frequency map and a threshold, return the set of keys whose frequency is >= threshold.
...
- **Sum of counts**: The sum of all values in the frequency map equals |s|.
...
- **Order preservation**: Elements in the output appear in the same relative order as in the input.
- **Completeness**: Every element of the input that is in the keep set appears in the output.
- **No fabrication**: The output is a subsequence of the input.

### Compositional properties (the hard part)
- **End-to-end contract propagation**: Calling CollectFiltered(s, FilterByFrequency(Frequencies(s), k)) produces a subsequence of s containing exactly those elements that appear at least k times in s.
